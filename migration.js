...
const generatePost = (num) =>{
    const title = faker.lorem.word({min:10,max:45});
    ...
    const uuId = uuIdv4();
    const statement =`INSERT INTO posts_table (title,owner, content, views, upload_date, uuId) VALUES ('${title}','${owner}', '${content}', ${views}, '${upload_date}', '${uuId}');`;
    // executeQuery(statement);
    return {sql:statement,data: {title,owner,content,views,upload_date,uuId}};
}
